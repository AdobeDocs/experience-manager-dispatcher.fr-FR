...
### Comment configurer le Dispatcher pour l’instance de création ?

Voir [Utilisation de Dispatcher avec une instance de création](dispatcher.md#using-a-dispatcher-with-an-author-server) pour connaître les étapes détaillées.
...
Oui, si l’ordinateur est suffisamment puissant. Toutefois, vous devez configurer Dispatcher et l’instance de publication AEM sur différents ordinateurs.
...
Voir les pages [Liste de contrôle de sécurité de Dispatcher](security-checklist.md) et [Liste de contrôle de sécurité d’AEM](https://experienceleague.adobe.com/fr/docs/experience-manager-64/administering/security/security-checklist).
...
**Réponse** : appelez `ResourceResolver.map()` pour générer une URL simple pour les requêtes GET et résoudre les problèmes de mise en cache de Dispatcher. La méthode map() code les deux-points `:` en caractères de soulignement et la méthode resolve() les décode au format lisible SLING JCR. Utilisez la méthode map() pour générer l’URL utilisée dans l’appel AJAX.
...
Voir la page [Réplication](https://experienceleague.adobe.com/fr/docs/experience-manager-65/content/implementing/deploying/configuring/replication#configuring-your-replication-agents).
