---
title: Configuration AEM Dispatcher
description: Découvrez comment configurer le Dispatcher AEM. Découvrez la prise en charge d’IPv4 et IPv6, des fichiers de configuration, des variables d’environnement et du nommage de l’instance. Découvrez comment définir des fermes de serveurs, identifier des hôtes virtuels, etc.
...
Le fichier de configuration contient une série de propriétés à une ou plusieurs valeurs qui contrôlent le comportement de Dispatcher AEM :
...
Par exemple, pour inclure le fichier myFarm.any dans la variable `/farms` La configuration utilise le code suivant :
...
Autre exemple : si vous créez une variable d’environnement nommée `PUBLISH_IP` qui stocke le nom d’hôte de l’instance de publication AEM, la configuration suivante de la fonction [`/renders`](#defining-page-renderers-renders) peut être utilisée :
...
* Utilisez une seule ferme de serveurs lorsque vous souhaitez que le Dispatcher d’AEM traite toutes vos pages web ou tous vos sites web de la même manière.
...
| [/filter](#configuring-access-to-content-filter) | Définit les URL auxquelles le Dispatcher AEM permet d’accéder. |
...
## Définition d’une page par défaut (IIS uniquement) - `/homepage` {#specify-a-default-page-iis-only-homepage}
...
Par défaut, Dispatcher AEM transfère les en-têtes HTTP standard vers l’instance AEM. Dans certains cas, vous pouvez transférer des en-têtes supplémentaires ou supprimer des en-têtes spécifiques :
...
La variable `/virtualhosts` définit une liste de toutes les combinaisons nom d’hôte et URI que Dispatcher accepte pour cette ferme de serveurs. Vous pouvez utiliser un astérisque (`*`) comme caractère générique. Valeurs de /`virtualhosts` utilisent le format suivant :
...
Par conséquent, vous devez placer l’hôte virtuel par défaut en haut de la page `virtualhosts` . Accordez-le dans la ferme de serveurs la plus élevée de votre `dispatcher.any` fichier .
...
La variable `/renders` définit l’URL à laquelle AEM Dispatcher envoie des demandes de rendu d’un document. La section d’exemple suivante `/renders` identifie une seule instance AEM pour le rendu :
...
L’exemple suivant `/renders` identifie une instance d’AEM qui s’exécute sur le même ordinateur que Dispatcher :
...
L’exemple suivant `/renders` La section répartit les demandes de rendu de manière égale entre deux instances AEM :
...
La fonction `getaddrinfo` renvoie une liste d’adresses IP. Dispatcher itère la liste des adresses jusqu’à établir une connexion TCP/IP. Par conséquent, la variable `ipv4` est importante lorsque le nom d’hôte du rendu est associé à plusieurs adresses IP. Et l’hôte, en réponse à la `getaddrinfo` , renvoie une liste d’adresses IP toujours dans le même ordre. Dans ce cas, vous devez utiliser la fonction `gethostbyname`, de sorte que l’adresse IP à laquelle Dispatcher se connecte soit randomisée.
...
>[!CAUTION]
>
>Voir [Liste de contrôle de sécurité de Dispatcher](security-checklist.md) pour d’autres considérations lors de la limitation de l’accès à l’aide d’AEM Dispatcher. Pour plus de détails sur la sécurité de votre installation AEM, lisez aussi la [Liste de contrôle de sécurité d’AEM](https://experienceleague.adobe.com/fr/docs/experience-manager-65/content/security/security-checklist#security).
...
La section d’exemple de filtre suivante entraîne AEM Dispatcher à refuser les demandes pour tous les fichiers. Refusez l’accès à tous les fichiers, puis autorisez l’accès à des zones spécifiques.
...
### Exemple `/filter` section {#example-filter-section}

Lors de la configuration du Dispatcher AEM, limitez autant que possible l’accès externe. L’exemple suivant offre un accès minimal aux visiteurs et visiteuses externes :
...
Depuis la version 4.1.5 de Dispatcher, utilisez la section `/filter` pour limiter les chaînes de requête. Il est recommandé d’autoriser explicitement les chaînes de requête et d’exclure l’allocation générique par le biais de `allow` des éléments de filtre.
...
Pour déterminer si l’accès en écriture anonyme est activé, lancez la commande suivante dans un terminal ou une invite de commande. Il ne doit pas être possible d’écrire des données sur le noeud.
...
Configurez Dispatcher AEM pour activer l’accès aux URL de redirection vers un microsite configurées pour vos pages AEM.
...
* `/file` : chemin d’accès au fichier local sur lequel Dispatcher stocke la liste des URL de redirection vers un microsite. Assurez-vous que Dispatcher AEM dispose d’un accès en écriture à ce fichier.
...
>[!NOTE]
>
>La valeur doit être le même chemin que la racine du document du serveur web afin que Dispatcher AEM et le serveur web traitent les mêmes fichiers.\
...
Le fichier stat n’a aucun contenu. Lorsque le contenu est mis à jour, l’AEM de Dispatcher met à jour l’horodatage. Le fichier stat par défaut est nommé `.stat` et est stocké dans le docroot. Dispatcher bloque l’accès au fichier stat.
...
La propriété `/serveStaleOnError` contrôle si Dispatcher renvoie des documents invalidés lorsque le serveur de rendu renvoie une erreur. Par défaut, lorsqu’un fichier stat est modifié et invalide le contenu mis en cache, AEM Dispatcher supprime le contenu mis en cache. Cette action est effectuée la prochaine fois qu’elle est demandée.

If `/serveStaleOnError` est défini sur `"1"`, Dispatcher ne supprime pas le contenu invalidé du cache. En d’autres termes, à moins que le serveur de rendu ne renvoie une réponse réussie. Une réponse 5xx d’AEM ou un délai d’expiration de connexion entraîne AEM Dispatcher à diffuser le contenu obsolète et à répondre avec l’état HTTP 111 (Échec de la revalidation).
...
* Le type indique si les documents qui correspondent au motif `glob` doivent être mis en cache. La valeur peut être `allow` (mettre le document en cache) ou `deny` (rendu du document).
...
>[!NOTE]
>
>Ne mettez pas en cache les groupes d’utilisateurs fermés. La raison est que les droits utilisateur ne sont pas vérifiés pour détecter les pages mises en cache.
...
* Tous les autres fichiers correspondant à la configuration `/invalidate` ne sont pas immédiatement supprimés. Ces fichiers sont supprimés lors de la requête suivante. Dans l’exemple : `/content/wknd.html` n’est pas supprimé ; il est supprimé lorsque `/content/wknd.html` est demandée.

Si vous proposez des fichiers de PDF et ZIP générés automatiquement pour téléchargement, vous devrez peut-être également invalider ces fichiers automatiquement. Voici un exemple de configuration :
...
Si un paramètre est ignoré pour une page, la page est mise en cache lorsqu’elle est demandée pour la première fois. Les requêtes suivantes de la page sont diffusées à la page mise en cache, quelle que soit la valeur du paramètre dans la requête.
...
* Pour mettre en cache une page, quelle que soit la requête contenant un paramètre d’URL, créez une propriété glob qui permet au paramètre (d’être ignoré).
...
Dans l’exemple suivant, Dispatcher ignore tous les paramètres, à l’exception du paramètre `nocache`. Par conséquent, Dispatcher ne met jamais en cache les URL de requête qui incluent la variable `nocache` parameter:
...
Dans le cadre de la `ignoreUrlParams` exemple de configuration, la requête HTTP suivante provoque la page **not** pour être mis en cache, car la variable `nocache` n’est pas ignoré :
...
>[!NOTE]
>
>Les caractères d’extension métacaractère de fichier ne sont pas autorisés. Pour plus d’informations, voir [Création de modèles pour les propriétés Glob](#designing-patterns-for-glob-properties).

>[!NOTE]
>
>Si vous avez besoin de l’AEM Dispatcher pour stocker et diffuser les en-têtes de réponse ETag d’AEM, procédez comme suit :
>
...
La propriété `mode` définit les autorisations de fichier appliquées aux nouveaux répertoires et fichiers du cache. La variable `umask` du processus d’appel limite ce paramètre. Il s’agit d’un nombre octal construit à partir de la somme d’une ou de plusieurs des valeurs suivantes :
...
2. Si le fichier n’a pas expiré, ou `/enableTTL` n’est pas configuré, les règles d’invalidation du cache standard sont appliquées, telles que les règles qui [`/statfileslevel`](#invalidating-files-by-folder-level) et [`/invalidate`](#automatically-invalidating-cached-files) définie. Ce flux signifie que Dispatcher AEM peut invalider les fichiers pour lesquels la durée de vie n’a pas expiré.

Cette nouvelle mise en oeuvre prend en charge les cas d’utilisation où les fichiers ont un délai d’activation plus long (par exemple, sur le réseau de diffusion de contenu). Mais ces fichiers peuvent toujours être invalidés même si le délai d’activation n’a pas expiré. Elle privilégie l’actualisation du contenu par rapport au taux d’accès au cache sur Dispatcher.
...
Chaque fois que Dispatcher AEM nécessite une page rendue, il utilise l’algorithme suivant pour sélectionner le rendu :
...
Pour déterminer la catégorie d’un URI, AEM Dispatcher compare l’URI à chaque modèle de catégorie jusqu’à ce qu’une correspondance soit trouvée. Dispatcher commence par la première catégorie de la liste et continue dans l’ordre. Par conséquent, placez d’abord les catégories avec des motifs plus spécifiques.
...
Par exemple, la pénalité est appliquée lorsque la connexion TCP/IP au nom d’hôte/port désigné ne peut pas être établie. La raison en est soit que l’AEM n’est pas en cours d’exécution (et ne l’écoute pas), soit qu’il s’agit d’un problème lié au réseau.
...
## Identification d’un dossier de connexion persistante - `/stickyConnectionsFor` {#identifying-a-sticky-connection-folder-stickyconnectionsfor}
...
La propriété `/retryDelay` définit le délai (en secondes) pendant lequel Dispatcher attend entre les séries de tentatives de connexion aux rendus de la batterie. Pour chaque tour, le nombre maximal de fois que Dispatcher AEM tente une connexion à un rendu est le nombre de rendus dans la ferme de serveurs.
...
Pour chaque tour, le nombre maximal de fois que Dispatcher AEM tente une connexion à un rendu est le nombre de rendus dans la ferme de serveurs. Par conséquent, le nombre maximal de tentatives de connexion de Dispatcher AEM est ( `/numberOfRetries`) x (nombre de rendus).
...
* Lorsqu’une requête à un rendu renvoie un état HTTP 50x (autre que 503), Dispatcher envoie une requête pour la page configurée pour la variable `health_check` .
   ...
   * Si le contrôle de l’intégrité renvoie un état HTTP 200, l’AEM Dispatcher renvoie l’erreur HTTP 500 initiale au client.
...
Ces messages peuvent être générés lorsque la variable `EINTR` se produit dans la variable `read more data` . Et c&#39;est la réception d&#39;un signal avant que des données n&#39;aient été reçues qui est la cause.
...
| `[ and ]` | Marque le début et la fin d’une classe de caractères. Les classes de caractères peuvent inclure une ou plusieurs plages de caractères et des caractères uniques.<br/>Une correspondance se produit si le caractère cible correspond à n’importe quel caractère de la classe de caractères ou d’une plage définie.<br/>Si le crochet fermant n’est pas inclus, le modèle ne produit pas de correspondance. | `*[o]men.html*`<br/> correspond à la requête HTTP suivante : <br/>.<ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li></ul><br/>Elle ne correspond pas à la requête HTTP suivante :<br/><ul><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul><br/> `*[o/]men.html*`<br/>Correspond aux requêtes HTTP suivantes : <br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul> |
| `-` | Il indique une plage de caractères. À utiliser dans des classes de caractères. En dehors d’une classe de caractères, ce caractère est interprété littéralement. | `*[m-p]men.html*`Correspond à la requête HTTP suivante : <br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li></ul>Elle ne correspond pas à la requête HTTP suivante :<br/><ul><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul> |
| `!` | Annule le caractère ou la classe de caractères qui suit. À utiliser uniquement pour annuler des caractères et des plages de caractères dans des classes de caractères. Équivalent au `^ wildcard` <br/>En dehors d’une classe de caractères, ce caractère est interprété littéralement. | `*[!o]men.html*`<br/> correspond à la requête HTTP suivante : <br/>.<ul><li>`"GET /content/geometrixx-outdoors/en/men.html"`</li></ul><br/>Elle ne correspond pas à la requête HTTP suivante : <br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"`</li></ul><br/>`*[!o!/]men.html*`<br/> Elle ne correspond pas à la requête HTTP suivante :<br/><ul><li>`"GET /content/geometrixx-outdoors/en/women.html"` ou `"GET /content/geometrixx-outdoors/en/men. html"`</li></ul> |
...
**Journaux pivotés ou redirigés Apache**

Si vous utilisez un **Apache** serveur web, vous pouvez utiliser la fonctionnalité standard pour les rotations de journaux, les journaux redirigés, ou les deux. Par exemple, à l’aide des journaux redirigés :
...
Un événement est également consigné lorsqu’un fichier correspondant à une règle de blocage est demandé :
...
Le fichier cible est contenu dans le cache. Cependant, un fichier stat plus récent peut l’invalider. Dispatcher supprime le fichier cible, le recrée à partir de la sortie et le diffuse.
* **mise en cache impossible : racine du document inexistante**
...
* **mise en cache impossible : l’extension de l’URL de la requête est manquante**\
  L’URL de la demande n’a pas d’extension ou un chemin suit l’extension du fichier, par exemple : `/test.html/a/path`.
* **mise en cache impossible : la demande doit être un GET ou un HEAD**
...
* **mise en cache impossible : le gestionnaire de session doit s’authentifier**\
  Un gestionnaire de session (la configuration contient une `sessionmanagement` ) gouverne le cache de la ferme de serveurs et la requête ne contient pas les informations d’authentification appropriées.
...
* **mise en cache impossible : l’URL de la requête est absente des règles du cache**\
...
Un gestionnaire de session (la configuration contient une `sessionmanagement` ) gouverne le cache de la ferme de serveurs et la session de l’utilisateur n’est pas ou n’est plus valide.
