...
description: Découvrez comment configurer Dispatcher. Découvrez la prise en charge d’IPv4 et d’IPv6, les fichiers de configuration, les variables d’environnement et l’attribution de noms à l’instance. Découvrez comment définir des batteries, identifier des hôtes virtuels, etc.
...
Par défaut, Dispatcher transfère les en-têtes HTTP standard vers l’instance AEM. Dans certains cas, vous souhaiterez peut-être transférer des en-têtes supplémentaires ou supprimer des en-têtes spécifiques :
...
Si `/serveStaleOnError` est défini sur `"1"`, Dispatcher ne supprime pas le contenu invalidé du cache, sauf si le serveur de rendu renvoie une réponse de succès. Une réponse 5xx d’AEM ou un délai d’expiration de connexion entraîne Dispatcher à diffuser le contenu obsolète et à répondre avec l’état HTTP 111 (Échec de la revalidation).
...
De tels messages peuvent être générés lorsque `EINTR` se produit dans la section `read more data`. Ceci est causé par la réception d’un signal avant que des données n’aient été reçues.
