...
Pour plus d’informations sur la configuration requise et sur les conditions préalables, consultez les [Plateformes prises en charge](https://experienceleague.adobe.com/fr/docs/experience-manager-64/deploying/introduction/technical-requirements).
...
* DISP-949 - Dispatcher renvoie 200 au lieu de 404 même si le filtre bloque la requête du POST.
...
* DISP-874 - Création d’une configuration Dispatcher pour activer ou désactiver l’implémentation de DISP-818 par le biais d’un indicateur `DispatcherRestrictUncacheableContent`. La valeur par défaut est Désactivé. Lorsqu’elle la valeur est Activé, elle supprime tous les en-têtes de mise en cache définis par mod expires pour le contenu ne pouvant être mis en cache. Ce paramètre est différent du comportement de la version 4.3.3 (où la valeur par défaut était Activé), mais identique aux versions antérieures à la version 4.3.3 (où la valeur par défaut était Désactivé). Maintenir la valeur par défaut Désactivé de `DispatcherRestrictUncacheableContent` est l’approche recommandée, de sorte que la mémoire cache du navigateur dispose d’une plus grande flexibilité. Lors de la mise à niveau de la version 4.3.3 vers 4.3.4, si vous souhaitez conserver le même comportement que dans la version 4.3.3, vous devez définir explicitement `DispatcherRestrictUncacheableContent` sur Activé.
...
* DISP-944 - Pré-charger les URL Vanity
...
* DISP-749 - Alpine Linux® Dispatcher se bloque avec le niveau de journal de trace
...
* DISP-825 - Consignez un message spécial lorsqu’il n’y a plus d’espace sur le disque.
* DISP-826 - Prend en charge les URI de récupération avec une chaîne de requête
...
* DISP-725 - Caractères de fin après la conversion silencieuse de chaînes en valeur sans nom
...
* DISP-730 - 404 lors de la tentative d’accès à un fichier d’en-tête sur Dispatcher
...
* DISP-706 - Lors de l’invalidation, Dispatcher suit des liens symboliques de référence qui peuvent provoquer une boucle infinie.
