...
description: Découvrez comment installer le module Dispatcher sur Microsoft® Internet Information Server, Apache Web Server, et le Sun Java™ Web Server-iPlanet.
contentOwner: User
converted: true
...
Utilisez la procédure suivante pour écrire les messages du journal Dispatcher dans un fichier journal plutôt que dans le journal des événements Windows. Configurez Dispatcher pour utiliser le fichier journal et accordez à IIS un accès en écriture au fichier.
...
>[!CAUTION]
>
>Si vous créez un fichier binaire Apache en compilant les fichiers source, assurez-vous d’activer la **`dynamic modules support`**. Cela peut être fait en utilisant l’une des options **partagées**. Au minimum, incluez le module `mod_so`.
>
...
| dispatcher.any | Exemple de fichier de configuration pour Dispatcher. |
| LISEZMOI | Fichier Lisezmoi contenant les instructions d’installation et les informations de dernière minute. **Note** : consultez ce fichier avant de commencer l’installation. |
...
| DispatcherDeclineRoot | Définit le refus ou l’acceptation des requêtes vers la racine « / » : <br/>**0** - accepte des requêtes vers « / ». <br/>**1** - les requêtes vers « / » ne sont pas gérées par Dispatcher. Utilisez mod_alias pour un mappage correct. |
...
>
>(la version 4.0.9 de Dispatcher a été publiée en 2011. Si vous utilisez une ancienne version, une mise à niveau vers une version récente de Dispatcher serait appropriée).
