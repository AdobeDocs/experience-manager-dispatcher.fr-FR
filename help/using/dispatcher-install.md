...
L’agent de réplication `Flush` par défaut dans l’instance de création est configuré de sorte qu’il n’envoie pas d’informations d’identification de sécurité avec les requêtes de purge. Le site web sur lequel vous utilisez le cache de Dispatcher doit donc autoriser l’accès anonyme.
...
### Enregistrer le type MIME JSON - IIS 8.5 et 10 {#registering-the-json-mime-type-iis-and}
...
### Consigner les messages IIS dans un fichier - IIS 8.5 et 10 {#logging-iis-messages-to-a-file-iis-and}

Utilisez la procédure suivante pour consigner des messages de journal de Dispatcher dans un fichier journal au lieu du journal des événements de Windows. Configurez le Dispatcher pour utiliser le fichier journal et accordez à IIS un accès en écriture au fichier.
...
### Serveur web Apache - Ajouter le module Dispatcher {#apache-web-server-add-the-dispatcher-module}
...
### Serveur web Apache - Configurer les propriétés SELinux {#apache-web-server-configure-selinux-properties}
...
>[!NOTE]
>
>Si vous utilisez une version personnalisée d’Apache, assurez-vous qu’Apache et Dispatcher sont compilés en utilisant la même version d’OpenSSL.<!-- URL has connection error [OpenSSL] (https://www.openssl.org/source/). -->
